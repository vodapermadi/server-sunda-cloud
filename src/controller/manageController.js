...
router.get("/",async (req,res) => {
    const data = await getAllData("manage")
...
router.get("/{id}",async(req,res) => {
    ...
    const result = await getSingleData("manage","id",id)
...
router.post("/insert",async (req,res) => {
    ...
    await insertData("manage",value,coloum)
...
router.post("/update/{id}",async(req,res) => {
    ...
    await updateData("manage",value,coloum,"id",id)

    return res.json(customResponse("success", 200))
})

export {
    router as ManageController
}